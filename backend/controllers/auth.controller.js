...
export const signup = async (req, res) => {
  const { email, password, name, cin, tel } = req.body;

  try {
    if (!email || !password || !name || !cin || !tel) {
      throw new Error("All fields are required");
    }

    const userAlreadyExists = await User.findOne({ $or: [{ email }, { cin }] });
